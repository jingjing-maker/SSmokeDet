...
static void generate_SSmokeNet_proposals(std::vector<GridAndStride> grid_strides, float* feat_blob, float prob_threshold, std::vector<Object>& objects)
{
...
static void decode_outputs(float* prob, std::vector<Object>& objects, float scale, const int img_w, const int img_h) {
        ...
        generate_SSmokeNet_proposals(grid_strides, prob,  BBOX_CONF_THRESH, proposals);
...
int main(int argc, char** argv) {
    ...
    if (argc == 4 && std::string(argv[2]) == "-i") {
        ...
        std::cerr << "run 'python3 SSmokeNet/deploy/trt.py -n SSmokeNet-{tiny, s, m, l, x}' to serialize model first!" << std::endl;
        std::cerr << "Then use the following command:" << std::endl;
        std::cerr << "./SSmokeNet ../model_trt.engine -i ../../../assets/dog.jpg  // deserialize file and run inference" << std::endl;
        return -1;
    }
