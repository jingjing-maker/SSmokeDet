...
Cpp file compile of SSmokeNet object detection base on [ncnn](https://github.com/Tencent/ncnn).  
SSmokeNet is included in ncnn now, you could also try building from ncnn, it's better.
...
For example, if you want to generate onnx file of SSmokeNet-s, please run the following command:
```shell
cd <path of SSmokeNet>
python3 tools/export_onnx.py -n SSmokeNet-s
```
Then, a SSmokeNet.onnx file is generated.
...
cd <path of ncnn>
cd build/tools/ncnn
./onnx2ncnn SSmokeNet.onnx SSmokeNet.param SSmokeNet.bin
```
...
```
will be printed. However, don't  worry!  C++ version of Focus layer is already implemented in SSmokeNet.cpp.

### Step4
Open **SSmokeNet.param**, and modify it.
...
../ncnnoptimize model.param model.bin SSmokeNet.param SSmokeNet.bin 65536
```

### Step6
Copy or Move SSmokeNet.cpp file into ncnn/examples, modify the CMakeList.txt, then build SSmokeNet

### Step7
Inference image with executable file SSmokeNet, enjoy the detect result:
```shell
./SSmokeNet demo.jpg
```
