...
echo "\"command\": \"$CXX SSmokeNet.cpp -o SSmokeNet ${INCLUDE_FLAG} ${LINK_FLAG}\"," >> compile_commands.json
echo "\"file\": \"$PWD/SSmokeNet.cpp\"," >> compile_commands.json
echo "}," >> compile_commands.json
echo "]" >> compile_commands.json
$CXX SSmokeNet.cpp -o SSmokeNet ${INCLUDE_FLAG} ${LINK_FLAG} ${BUILD_FLAG}

echo "build success, output file: SSmokeNet"
if [[ $CXX =~ "android" ]]; then
    ...
    echo "adb push/scp ./SSmokeNet SSmokeNet_s.mge android_phone"
    echo "adb push/scp ../../../assets/dog.jpg android_phone"
    echo "adb/ssh to android_phone, then run: LD_LIBRARY_PATH=. ./SSmokeNet SSmokeNet_s.mge dog.jpg cpu/multithread <warmup_count> <thread_number> <use_fast_run> <use_weight_preprocess>"
else
    echo "try command to run: LD_LIBRARY_PATH=$MGE_INSTALL_PATH/lib/:$OPENCV_INSTALL_LIB_PATH ./SSmokeNet SSmokeNet_s.mge ../../../assets/dog.jpg cuda/cpu/multithread <warmup_count> <thread_number> <use_fast_run> <use_weight_preprocess>"
fi
